...
class FileButtonGroup(QWidget):
    def __init__(self, file_name,file,file_type ,parent=None):
        ...
        self.button.setStyleSheet('background: white;font: 12px "Microsoft YaHei"; border-radius: 10px;border: 1px solid #ccc;')
...
class ProcessTask(CardWidget):
    def __init__(self, text, parent=None):
        ...
        # 输入框
        self.docx_rename = RenameLineEdit(self)
        # 获取输入框内容
        self.docx_rename.clicked.connect(self.docx_rename.showBottomTip)
    ...
    def mainTask(self):
        ...
        for image_group, image_path in self.parentObject.drop_upload.input_image_map.items():
            text = image_group.filename_edit.toPlainText()  # 获取输入框内容，并去除两端的空白字符
            # 根据输入框和图片路径的情况，分别处理
    ...
    def toPDFTask(self):
        ...
        self.mainTask()
        self.doc.save(os.path.join(_Config.get_app_path(), "temp.docx"))
        """启动文件转换线程，处理后台转换任务"""
        self.worker = ConvertFileWorker(f"{_Config.get_app_path()}/temp.docx", f"{formatPath}.pdf",2)
    ...
    def add_result_list(self,file,_type="docx"):
        ...
        # file = Document("/Users/yezery/Desktop/AUTOSYBG 2/view/22数媒6_叶正睿_22054181影视创意设计实验报告7期末大作业.docx")
        Config = get_config().config
        formatName = f"{Config['COURSE']}_{Config['ID']}_{Config['NAME']}_{self.docx_rename.text()}.{_type}"
...
class AppInterface(SmoothScrollArea):
    ...
    def __init__(self, text: str, parent=None):
        ...
        # 创建左侧滚动区域，放入 SummaryInterface
        left_scroll_area = SmoothScrollArea(self)
        left_scroll_area.setWidgetResizable(True)
        left_scroll_area.setScrollAnimation(Qt.Vertical, 400, QEasingCurve.OutQuint)
        left_scroll_area.setScrollAnimation(Qt.Horizontal, 400, QEasingCurve.OutQuint)
        ...
        self.summary_widget.setFixedHeight(300)
        ...
        # 将左侧和右侧区域添加到主布局
        main_layout.addWidget(left_widget, 2)  # 让左边区域可以滚动
        main_layout.addWidget(self.right_scroll_area, 2)
